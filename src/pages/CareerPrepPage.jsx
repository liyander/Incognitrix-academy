...
function CareerPrepPage() {
  ...
  return (
    <main className="rounded-xl min-h-screen bg-surface px-4 pb-20 pt-24 sm:px-6 lg:px-10">
      <div className="mx-auto max-w-7xl space-y-7">
        <header className="rounded-xl border-l-4 border-primary bg-surface-container-lowest px-6 py-7 md:px-9 md:py-9">
          <p className="font-headline text-[10px] font-bold tracking-normal text-primary">
            AI Interview Lab
          </p>
          <div className="mt-3 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <h1 className="font-headline text-4xl font-black md:text-5xl">Career Prep</h1>
            ...
            {session ? (
              <button
                className="rounded-xl inline-flex items-center justify-center gap-2 border border-outline-variant bg-surface-container-high px-5 py-3 font-headline text-[10px] font-bold tracking-normal hover:border-primary"
        ...
        {error ? (
          <div className="rounded-2xl border-l-4 border-error bg-error/10 px-5 py-4 text-sm text-error" role="alert">
            {error}
          </div>
        ) : null}
        ...
        {isLoading ? (
          <div className="rounded-xl flex min-h-80 items-center justify-center bg-surface-container-lowest">
            <div className="text-center">
              ...
              <p className="mt-3 font-headline text-[10px] font-bold tracking-normal text-on-surface-variant">Loading interview workspace</p>
            </div>
          </div>
        ) : session ? (
          <section className="grid min-w-0 gap-6 xl:grid-cols-[18rem_minmax(0,1fr)]">
            <aside className="rounded-2xl min-w-0 bg-surface-container-lowest p-5 xl:sticky xl:top-24 xl:self-start">
              <p className="font-headline text-[10px] font-bold tracking-normal text-secondary">Question Map</p>
              <h2 className="mt-2 break-words font-headline text-xl font-black">{session.roleTitle}</h2>
              ...
              <div className="mt-2 flex justify-between font-label text-[10px] tracking-normal text-on-surface-variant">
            ...
            <div className="min-w-0 space-y-5">
              {session.status === 'completed' && sessionSummary ? (
                <div className="rounded-2xl border-l-4 border-secondary bg-surface-container-lowest p-6">
                  <p className="font-headline text-[10px] font-bold tracking-normal text-secondary">Session Complete</p>
                ...
                <article className="min-w-0 border border-outline-variant bg-surface-container-lowest">
                  <div className="rounded-2xl border-b border-outline-variant px-5 py-5 md:px-7">
                    <div className="flex flex-wrap items-center gap-3">
                      ...
                      <span className="rounded-full bg-secondary/10 px-3 py-1 font-headline text-[10px] font-bold tracking-normal text-secondary">
                        {activeQuestion.competency || 'Role readiness'}
                      </span>
                      {activeQuestion.questionType === 'company_past' ? (
                        <span className="rounded-full bg-primary/10 px-3 py-1 font-headline text-[10px] font-bold tracking-normal text-primary">Sourced interview pattern</span>
                      ) : null}
                    </div>
                    ...
                    {activeQuestion.source ? (
                      <div className="rounded-xl mt-5 border-l-2 border-secondary bg-surface-container-low px-4 py-3">
                        <p className="font-label text-[10px] font-bold tracking-normal text-secondary">Public source</p>
                        <a
                  ...
                  <div className="px-5 py-5 md:px-7 md:py-7">
                    <label className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant" htmlFor={`answer-${activeQuestion.id}`}>
                      Your answer
                    </label>
                    <textarea
                      className="rounded-2xl mt-3 min-h-52 w-full resize-y border border-outline-variant border-l-2 border-l-primary bg-surface-container-highest px-4 py-4 text-sm leading-7 text-on-surface outline-none focus:border-primary disabled:opacity-80"
                    ...
                    {!activeQuestion.answered ? (
                      <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <button
                          className="rounded-xl inline-flex min-h-12 items-center justify-center gap-2 bg-primary px-6 font-headline text-[10px] font-bold tracking-normal text-on-primary disabled:cursor-not-allowed disabled:opacity-50"
                  ...
                  {activeQuestion.answered ? (
                    <div className="rounded-2xl border-t border-outline-variant bg-surface-container-low px-5 py-6 md:px-7">
                      <div className="flex flex-wrap items-center gap-4">
                        ...
                        <div>
                          <p className="font-headline text-sm font-black">{activeQuestion.verdict}</p>
                      ...
                      <div className="mt-6 grid gap-5 lg:grid-cols-2">
                        <section className="border-l-2 border-secondary pl-4">
                          <h3 className="font-headline text-[10px] font-bold tracking-normal text-secondary">What worked</h3>
                        ...
                        <section className="border-l-2 border-primary pl-4">
                          <h3 className="font-headline text-[10px] font-bold tracking-normal text-primary">Improve next</h3>
                          <ul className="mt-3 space-y-2 text-sm leading-6 text-on-surface-variant">
                      ...
                      <section className="mt-6 border-t border-outline-variant pt-5">
                        <h3 className="font-headline text-[10px] font-bold tracking-normal text-secondary">Model answer</h3>
          ...
          <section className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]">
            <div className="rounded-2xl min-w-0 bg-surface-container-lowest p-6 md:p-8">
              <div className="flex flex-wrap gap-2 border-b border-outline-variant pb-6">
                <button
                  className={`px-5 py-3 font-headline text-[10px] font-bold tracking-normal ${sourceType === 'recommended' ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface-variant'}`}
                ...
                <button
                  className={`px-5 py-3 font-headline text-[10px] font-bold tracking-normal ${sourceType === 'custom' ? 'bg-primary text-on-primary' : 'bg-surface-container-high text-on-surface-variant'}`}
              ...
              {sourceType === 'recommended' ? (
                <div className="mt-6">
                  <label className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant" htmlFor="recommended-job">Recommended job</label>
                  {jobs.length ? (
                    <select
                      className="rounded-xl mt-3 min-h-14 w-full border border-outline-variant border-l-2 border-l-secondary bg-surface-container-highest px-4 text-sm outline-none focus:border-secondary"
                      id="recommended-job"
                  ...
                  {jobs.find((job) => String(job.recommendationId) === recommendationId) ? (
                    <div className="mt-5 border-l-2 border-secondary pl-4">
                      <p className="font-headline text-lg font-black">{jobs.find((job) => String(job.recommendationId) === recommendationId)?.title}</p>
                ...
                <div className="mt-6 grid gap-5 sm:grid-cols-2">
                  <label className="block">
                    <span className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant">Role title</span>
                    <input className="rounded-xl mt-2 min-h-13 w-full border border-outline-variant border-l-2 border-l-primary bg-surface-container-highest px-4 outline-none focus:border-primary" onChange={(event) => setCustomRole((current) => ({ ...current, roleTitle: event.target.value }))} placeholder="SOC Analyst" value={customRole.roleTitle} />
                  </label>
                  <label className="block">
                    <span className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant">Company (optional)</span>
                    <input className="rounded-xl mt-2 min-h-13 w-full border border-outline-variant border-l-2 border-l-secondary bg-surface-container-highest px-4 outline-none focus:border-secondary" onChange={(event) => setCustomRole((current) => ({ ...current, company: event.target.value }))} placeholder="Company name" value={customRole.company} />
                  </label>
                  <label className="block sm:col-span-2">
                    <span className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant">Job description</span>
                    <textarea className="rounded-xl mt-2 min-h-40 w-full resize-y border border-outline-variant border-l-2 border-l-primary bg-surface-container-highest px-4 py-3 outline-none focus:border-primary" onChange={(event) => setCustomRole((current) => ({ ...current, jobDescription: event.target.value }))} placeholder="Paste the role description and responsibilities..." value={customRole.jobDescription} />
                  </label>
                  <label className="block sm:col-span-2">
                    <span className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant">Requirements</span>
              ...
              <div className="mt-7 border-t border-outline-variant pt-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                  <label className="block sm:w-64">
                    <span className="font-headline text-[10px] font-bold tracking-normal text-on-surface-variant">Number of questions</span>
                    <div className="mt-2 flex items-center gap-3">
                      ...
                      <output className="rounded-xl flex h-12 w-14 shrink-0 items-center justify-center bg-surface-container-high font-headline text-xl font-black text-primary">{questionCount}</output>
                    </div>
                  </label>
                  <button
                    className="rounded-xl inline-flex min-h-13 items-center justify-center gap-2 bg-primary px-7 font-headline text-[10px] font-bold tracking-normal text-on-primary disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <aside className="rounded-2xl bg-surface-container-lowest p-5 md:p-6">
              <p className="font-headline text-[10px] font-bold tracking-normal text-secondary">Recent Sessions</p>
                ...
                {sessions.length ? sessions.map((item) => (
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <p className="min-w-0 break-words font-headline text-sm font-black">{item.roleTitle}</p>
                      <span className={`shrink-0 font-label text-[9px] font-bold tracking-normal ${item.status === 'completed' ? 'text-secondary' : 'text-primary'}`}>{item.status}</span>
                    </div>
      ...
      {(isGenerating || isEvaluating) ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/70 px-5 backdrop-blur-sm" role="status">
          <div className="rounded-2xl w-full max-w-md border-t-4 border-primary bg-surface-container-lowest p-8 text-center shadow-2xl">
            <span className="material-symbols-outlined animate-spin text-5xl text-secondary">progress_activity</span>
            <h2 className="mt-5 font-headline text-2xl font-black">{isGenerating ? 'Preparing your interview' : 'Reviewing your answer'}</h2>
