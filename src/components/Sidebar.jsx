    ...
    <>
      ...
      >
        ...
        {config.features.newMissionButton ? (
          <div className="mt-3 rounded-2xl bg-blush p-4 shrink-0">
            <p className="font-headline text-base font-extrabold text-on-blush leading-tight">
              Go further
              <br />
              with Pro
            </p>
            <p className="mt-1 font-body text-xs text-on-blush/80">
              Unlock every course, project and mentor session.
            </p>
            <NavLink
              className="mt-3 inline-flex items-center gap-1 font-headline text-xs font-bold text-on-blush hover:underline"
              onClick={onClose}
              to="/learn"
            >
              Explore plans
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 glass-nav border-t border-outline-variant/60 flex items-center gap-2 overflow-x-auto px-3 py-2 z-50">
        {navItems.map((item) => (
          ...
          >
            <span className="material-symbols-outlined">{item.icon}</span>
            <span className="font-headline text-[9px] font-bold">{item.label}</span>
          </NavLink>
        ))}
      </nav>
    </>
